...
while [ $i -lt 9 ]; do
	...
	j=0
	while [ $j -lt 9 ]; do
		echo ${keyLocation[j]}$keyName ubuntu@${machine_aws[i]}

		sleep 3s
		if [ $j -lt 9 ]
		then
			echo ${keyLocation[j]}$keyName ubuntu@${machine_aws[j]}
			ssh -i ${keyLocation[j]}$keyName ubuntu@${machine_aws[j]} "iperf3 -c ${machine_google[i]} -i 1 -t $time > combined_bandwidth_google_${name[i]}_${nameAws[j]}.txt"&
			ssh -i ${keyLocation[j]}$keyName ubuntu@${machine_aws[j]} "ping -w $time ${machine_google[i]} > combined_latency_google_${name[i]}_${nameAws[j]}.txt"
			sleep 10s
			j=$[$j+1]
		fi
	done
